...
function get_list_city($razdel_link)
        ...
        $cachedDatasas = returnResultCache(3600000, 'list_filter_city1', 'get_list_city', $razdel_link);
...
// данные из кеша
       ...
       if (!array_key_exists($selected_reg, $list_regions)) { // такого города нет выводим 404
           ...
           if($selected_reg == 1) {
               ...
               $APPLICATION->SetPageProperty("tags", $nomberstr . "в реестр зарегистрированных адвоката адвокат" . $reg . "России РФ");
               $APPLICATION->SetPageProperty("keywords_inner", "в реестр зарегистрированных адвоката адвокат" . $reg . "России РФ");
               $APPLICATION->SetPageProperty("keywords", $nomberstr . "в реестр зарегистрированных адвоката адвокат" . $reg . "России РФ");
               $APPLICATION->SetPageProperty("description", $nomberstr . "Реестр зарегистрированных адвокатов" . (rtrim($reg)) . ", оказывающие профессиональные услуги" . $reg . "в России");
               $APPLICATION->SetTitle("Реестр адвокатов" . $reg);
               $APPLICATION->SetPageProperty("title", $nomberstr . "Реестр адвокатов" . $reg . " Российской Федерации");
               $APPLICATION->SetPageProperty("canonical", "https://jur24pro.ru".$razdel_link . $code );
       }

        ?>
<h1 class="main-title">Реестр адвокатов<?=(rtrim($reg));?></h1>
...
<div class="filter__user">
 ...
 function get_list_user($data_array)
 {
     ...
     $rsUsers->NavStart(9, false, intval($PAGEN_1));// разбиваем постранично по 50 записей
 ...
 // кеширование пользователей
    $data_result_user = returnResultCache(3600000, 'reestr_advokaty/list_filter_user'.$region_user.'_'.(intval($_GET['PAGEN_1'])), 'get_list_user',
        array(
    ...
    <div class="user_item col-md-4 col-sm-6 col-xs-12">
        <div class="filter_reg_user block-shadow">
            ...
            <div class="reg_user_content">
                ...
                <p>
                    <strong>Подразделение(адрес)</strong><br>
                    <?
