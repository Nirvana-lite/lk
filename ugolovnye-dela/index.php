...
if( count($page_array) == 2 ){
    ...
    if( ctype_digit($id_url) ){ // состоит из цифр
        ...
        if($ar_res = $res->Fetch()){
            if(trim($ar_res['LIST_PAGE_URL'], '/') != 'ugolovnye-dela'){
                show404();
            }else {
...
metatagrazdel(
    'ugolovnye-dela', // link
    'Приговоры по уголовным делам судов Российской Федераци', // title
    '',//SetTitle
    '', // tags
    'приговоры, уголовным, делам, судов', // keywords
    'Приговоры по уголовным делам судов РФ', // description
    'Приговоры по уголовным делам ' // h1
);
