...
if($arParams['arUserField']["SETTINGS"]["DISPLAY"] != "CHECKBOX")
{
	...
	{
		?>
		<select class="form-control<?=$arParams["classerror"]; ?>"  style="display:<?=$arParams["styleList"]; ?>;" name="<?echo $arParams["arUserField"]["FIELD_NAME"]?>" size="1" <?=($arParams["arUserField"]["EDIT_IN_LIST"]!="Y"? ' disabled="disabled" ':'')?> >
		<?
		$bWasSelect = false;
		foreach ($arParams["arUserField"]["USER_TYPE"]["FIELDS"] as $key => $val)
		{
