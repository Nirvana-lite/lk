function changeTab(e) {
    if (e === 2) {
        if ($('#tab2').hasClass('hide')) {
            ...
            let formData = new FormData(form_res);

            $('#tab_name').text(formData.get('name'));
            $('#tab_preview').text(formData.get('description'));

            let ifr = $('iframe#detail_text_ifr').contents();
            let detail_text = ifr.find('#tinymce').html();

            $('#tab_more-text').html(detail_text);

            let tagsArray = formData.get('tags').split(', ');
            tab_tags.innerHTML = '';

            for (let t = 0; t < tagsArray.length; t++) {
                let tag_a = document.createElement('a');
                tag_a.innerText = tagsArray[t];
                tab_tags.append(tag_a);
            }
        }
    } else {
...
function previewFile() {
    ...
    var tab_preview = document.querySelector('.detail_picture');
    var file    = document.getElementById('image').files[0];
    var reader  = new FileReader();

    reader.onloadend = function () {
        preview.src = reader.result;
        tab_preview.src = reader.result;
    };
...
function createAnsw(answ) {
    ...
    if (answ === 'done'){
        ...
        div.innerHTML = '<p>Текст отправлен на модерацию.</p><p>После успешной проверки Ваша статья опубликуется.</p>';
    }else{
...
function ascDel(){
    ...
    div1.className = 'delQuestion';
    div1.innerHTML = '<p>Точно удалить?</p><button onclick="delItem()">Да</button><button onclick="closeDiv()">Нет</button>'
    document.body.append(div1);
}

function closeDiv(eld) {
document.querySelector('.delQuestion').remove();
}

function delItem(){
    $(function () {
        let componentPath = BX.message('componentPath');
        let elem =  BX.message('element');
        let formData = new FormData();
        formData.append('id',elem);
        formData.append('del',true);
        axios.post(`${componentPath}/saveInfo.php`, formData)
            .then((response) => {
                location.reload();
...
$(function () {
    ...
    $("#form_res").on('submit', function (event) {
